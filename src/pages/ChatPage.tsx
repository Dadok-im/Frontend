    ...
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-gradient-to-br from-[#4A5D73] via-[#5C6373] to-[#A3B8C6] text-[#F5F7FA]">
      ...
      <nav className="relative z-10 flex items-center justify-center border-b border-[rgba(163,184,198,0.35)] bg-[rgba(245,247,250,0.08)] px-4 py-5 backdrop-blur-2xl shadow-lg sm:px-5 sm:py-6">
        ...
        <div className="absolute right-4 top-1/2 flex -translate-y-1/2 flex-wrap items-center gap-2 text-[11px] sm:right-6 sm:text-xs md:text-sm">
          {connectionStatus && (
            <span
              className={`rounded-full border px-3 py-1 font-medium shadow-inner transition duration-200 ${statusStyles[connectionStatus] ?? "border-[rgba(163,184,198,0.45)] bg-[rgba(245,247,250,0.12)] text-[#F5F7FA]/80"}`}
            >
      ...
      <div className="relative z-10 mx-auto flex w-full flex-1 flex-col px-4 pb-6 pt-4 sm:pb-8 sm:pt-6">
        <div
          className={`chat-container ${darkMode ? "dark" : "light"} relative mx-auto flex w-full max-w-md flex-1 flex-col overflow-hidden rounded-3xl border shadow-2xl backdrop-blur-2xl sm:max-w-lg md:max-w-xl ${
            darkMode
              ? "border-[rgba(74,93,115,0.35)] bg-[rgba(32,40,49,0.85)]"
              : "border-[rgba(163,184,198,0.45)] bg-[rgba(245,247,250,0.12)]"
          }`}
        >
          <ChatHeader
            menuOpen={menuOpen}
